use crate::v3ds_common::{FunctionData, RawFunctionData, V3dsCmbHeader, FE14_EVENTS};
...
fn read_shift_jis_string(data: &[u8], start: usize) -> anyhow::Result<String> {
    if start > data.len() {
...
fn read_function(
    ...
    Ok(FunctionData {
        ...
        frame_size: raw_function_data.frame_size,
        name,
        args,
        code,
    })
...
impl Opcode {
    fn read_v3ds_opcode(
        ...
        match opcode {
            ...
            0xB => Ok((addr, Opcode::VarAddr(cursor.read_u8()? as u16))),
            0xC => Ok((addr, Opcode::VarAddr(cursor.read_u16::<BigEndian>()?))),
            ...
            0x46 => Ok((addr, Opcode::CallById(cursor.read_u8()?))),
            0x47 => Ok((
                addr,
                Opcode::CallByName(
